...
def sln_eval(
    ...
    try:
        if method == "cot":
            ...
            pred = safe_execute_turbo(
                postprocess_code(sln)
            )  # postprocess_code will remove backticks from the code. This is applied only for p2c, I keep that bias in order to keep the experiments' consistency
    except:
        pred = None
...
def main(
    ...
    train_samples = get_k_train_shots(
        k=num_train_sample, train_f=dataset_f, heuristics=heuristics
    )
    ...
    for method in ["p2c"]:  # ['cot', 'pal', 'p2c']:
        ...
        if method == "cot":
            kwargs = dict(
                cot_temperature=verbal_T, backbone=backbone, n=n_llmquery, seed=seed
            )
            query_f = partial(f, **kwargs)
        elif method == "pal":
            kwargs = dict(
                pal_temperature=verbal_T, backbone=backbone, n=n_llmquery, seed=seed
            )
            query_f = partial(f, **kwargs)
        elif method == "p2c":
            kwargs = dict(
        ...
        for row in tqdm(train_samples, desc=method):
            try:
                ...
                eval_pred_lst = [
                    sln_eval(sln=sln, ans=ans, method=method) for sln in slnlst
                ]
                eval_lst = [
                    eval_ for eval_, pred in eval_pred_lst if pred is not None
                ]  # drop when answer wasn't parseable
                if eval_lst.count(True) < eval_lst.count(False) or dbg:  # majority
                    row[
                        "fail_freq"
                    ] = f"{backbone}: {eval_lst.count(False)}/{n_llmquery}"
                    row["fail_preds"] = [
                        eval_pred[-1]
                        for eval_pred, eval in zip(eval_pred_lst, eval_lst)
                        if not eval
                    ]
                    ...
                    if method == "p2c":
                        row["good_solutions"] = [
                            f"{plan}\n{sln}".strip()
                            for sln, e in zip(slnlst, eval_lst)
                            if (e) and (sln is not None)
                        ]
                        row["fail_solutions"] = [
                            f"{plan}\n{sln}".strip()
                            for sln, e in zip(slnlst, eval_lst)
                            if (not e) and (sln is not None)
                        ]
                    else:
                        row["good_solutions"] = [
                            sln
                ...
                if len(method2failed_questions[method]) == n_candids:
                    print(
                        f"at {row['idx']=}, gathering candids are done ({n_candids=}, {num_train_sample=})"
                    )
                    break
            except Exception as e:
                print(e)
...
def get_k_train_shots(
    k: int = 100, train_f: str = "gsm8k_train.jsonl", heuristics: str = "wordcount"
):
